...
function sortCarInventory(array) {
  /* code here */
  return array.sort(function(string_1, string_2){
    return string_1.car_model.localeCompare(string_2.car_model);
  });
...
function getModelYears(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    year_array.push(array[i].car_year);
  }
  return year_array;
}
...
function getOlderCars(array, max_year) {
  ...
  for (let i = 0; i < array.length; i++){
    if (array[i].car_year <= max_year){
      old_car_array.push(array[i]);
    }
  }
  return old_car_array;
}
